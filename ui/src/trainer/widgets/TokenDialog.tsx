...
const TokenDialog = ({
  ...
  const options = Object.keys(typeOptions).map((code: string) => (
    <MenuItem key={code} value={code}>
      {
        <span style={{ color: typeOptions[code].color }}>
          {typeOptions[code].label}
        </span>
      }
    </MenuItem>
  ));
  ...
  const selectWidgets = [
    ...options,
    actions.length > 0 && <Divider key="divider" />,
    ...actions
  ];

  const onSelect = (value: string) => {
    if (value == joinRightAction) {
      onJoinRight!();
      return;
    }
    if (value == joinLeftAction) {
      onJoinLeft!();
      return;
    }
    if (value == deleteAction) {
      onDelete!();
      return;
    }
